...
def draw_tsne(extractor, classifier, src_data_loader, tgt_data_loader, res_dir, idx, title, separate=True):
    ...
    #         plt.close()
        ...
        for data, label in zip(src_embeddings_2d, src_y):
            ...
            plt.scatter(x, y, label=label, marker='o', s=10, color=c)
        ...
        for data, label in zip(tgt_embeddings_2d, tgt_y):
            ...
            plt.scatter(x, y, label=label, marker='^', s=10, color=c)
    ...
    # 较为杂乱，不太容易区分
        ...
        for data, label in zip(src_embeddings_2d, src_y):
            ...
            y = data[1]
            c = cm.rainbow(int(255 * label / len(np.unique(src_y))))
            # plt.text(x, y, label, fontsize=6, bbox=dict(boxstyle='circle', fc=c))
            # plt.scatter(x, y, label=label, marker=marker_list[label], s=200, color=source_color)
            plt.scatter(x, y, label=label, s=200, color=source_color)
        plt.savefig('{}/{}_tsne_src.png'.format(res_dir, idx), dvi=300)
        print('save tsne result to {}/{}_tsne_src.png'.format(res_dir, idx))
        plt.close()
        ...
        for data, label in zip(tgt_embeddings_2d, tgt_y):
            ...
            plt.scatter(x, y, label=label, marker='^', s=200, color=target_color)

        # plt.legend()
        plt.savefig('{}/{}_tsne_tgt.png'.format(res_dir, idx), dvi=300)
        print('save tsne result to {}/{}_tsne_tgt.png'.format(res_dir, idx))
        plt.close()
